#!/usr/bin/env python3
"""
ZORK RPG - Multiplayer SSH Server (NO PASSWORD VERSION)
...
async def handle_client(process):
    """Handle one SSH client connection - NO PASSWORD AUTH"""
    global game_server
...
async def start_server(host='0.0.0.0', port=2222, config_path='config'):
    """Start the multiplayer SSH server - NO PASSWORD REQUIRED"""
    global game_server
...
╚════════════════════════════════════════════════╝
    ...
    print("\n⚡ PASSWORDLESS MODE - Just press Enter when connecting!")
    print("\nPlayers can connect with:")
    print(f"   ssh -o PubkeyAuthentication=no -o PreferredAuthentications=none -p {port} player@{host}")
    print(f"   OR just: ssh -p {port} player@{host}  (press Enter at password prompt)")
    print("\nPress Ctrl+C to stop the server.\n")
    
    try:
        # Create server with no authentication
        await asyncssh.create_server(
            lambda: asyncssh.SSHServer(),  # Simple server, no auth
            host,
            port,
...
def main():
    ...
    parser = argparse.ArgumentParser(description='ZORK RPG Multiplayer SSH Server (No Password)')
